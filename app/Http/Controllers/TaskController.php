...
class TaskController extends Controller
{
   ...
   public function task_update(Request $request){
    ...
    if($task){
        ...
        return redirect()->to('/tarefas')->with('msg','Tarefa actualizada com sucesso.');
    }else{
        
        return redirect()->back()->with('error','Erro ao actualizar tarefa.');
    }
...
// Retornar pagina par agerar relatorio / com filtro
   ...
   public function other_weekend(){
    try{
        ...
        $days = DB::select('select day,time from controls');

        if(count($days) > 0){
            
            $new_weekend = DB::update('update controls set day = ?',[$new_day]);
        }else{
            
            $new_weekend = DB::insert('insert into  controls(day,time) values(?,?)',[$new_day,'16:00']);
        }
   ...
   public function user_root(){
    ...
    if($user->count() > 0){
        ...
        DB::insert('insert into  controls(day,time) values(?,?)',[date('d',strtotime('7 days')),'16:00']);
        ...
        $time = '16:30';
       DB::insert('insert into controls(day,time) values(?,?)',[$day,$time]);
        User::create([
            'name'=>'ADMIN',
            'level'=>'admin',
